...
class LinkedList:
    ...
    def reverse_iterative(self, head):
        ...
        cur = self.head_
        while cur:
            nex = cur.next #point to next
            cur.next = prev #makes the next node point previous one
            prev = cur #previous node becomes current
            cur = nxt #current node is updated to next
            #set head to equal last node
        self.head = prev 
